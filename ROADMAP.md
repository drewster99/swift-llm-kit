...
2. **`claude-sonnet-5` and `claude-fable-5` are missing `requiresAdaptiveThinking`.** The payload (`thinking.types.enabled.supported == false`) and the docs (*"Manual extended thinking… is not supported and returns a 400 error"*) both say they need it; only opus-4-7/4-8 have it. Latent — fires only if thinking is enabled. The fix is to derive it, not to type another JSON entry.
...
5. **The effort allowlist is a vendor-blind union missing `none`.** `validateConfigurations` has `let validEfforts: Set<String> = ["minimal", "low", "medium", "high", "xhigh", "max"]` — a local variable, the only definition of effort levels in the codebase. It rejects `none` (a legal OpenAI value) and accepts `minimal` on Claude (which 400s). Per-model `validEffortLevels` replaces it.
